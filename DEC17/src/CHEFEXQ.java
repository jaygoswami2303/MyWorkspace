...
public class CHEFEXQ {
	...
	static class SegmentTree {
		...
		int getXor(int s, int e) {
			return getXorUtil(0, n-1, s, e, 0);
		}
		
		int getXorUtil(int s, int e, int qs, int qe, int i) {
			if(qs<=s && qe>=e)
				return tree[i];
			...
			int mid = s + (e-s)/2;
			return getXorUtil(s, mid, qs, qe, 2*i+1) ^ getXorUtil(mid+1, e, qs, qe, 2*i+2);
		}
