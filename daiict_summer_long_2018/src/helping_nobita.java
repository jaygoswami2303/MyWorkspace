...
public class helping_nobita {
	...
	public static void main(String[] args) {
		...
		for(int i=0;i<(n+1)/2;i++) {
			flag += i;
			ans = (ans%mod + (ar.get(2*i)%mod *(( ((i+1)%mod*n%mod)%mod - (2%mod*flag%mod)%mod )%mod + mod)%mod)%mod )%mod;
			if((2*i+1)<n)
				ans = (ans%mod + (ar.get(2*i+1)%mod *(( ((i+1)%mod*n%mod)%mod - (2%mod*flag%mod)%mod )%mod + mod)%mod)%mod )%mod;
		}
		pw.print(ans%mod);
		pw.close();
	}
