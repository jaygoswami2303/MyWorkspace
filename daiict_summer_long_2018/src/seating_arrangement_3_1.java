...
public class seating_arrangement_3_1 {
	...
	public static void main(String[] args) {
		...
		if(m%3!=0) {
			...
			if(current=='R')
				...
				foundT = true;
			for(int i=0;i<m/3 && correct;i++) {
				for(int j=0;j<n && correct;j++) {
					if(s[j].charAt(i)!=current)
						correct = false;
				}
			}
			current = s[0].charAt(m/3);
			if(current=='R') {
				if(foundR)
					correct = false;
				else
			...
			for(int i=m/3;i<2*m/3 && correct;i++) {
				for(int j=0;j<n && correct;j++) {
					if(s[j].charAt(i)!=current)
						correct = false;
				}
			}
			current = s[0].charAt(2*m/3);
			if(current=='R') {
				if(foundR)
					correct = false;
				else
